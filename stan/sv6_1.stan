...
transformed parameters {
  ...
  // tau[1] = sigma;
  h[1] = sigma*v[1];
  s[1,] = [exp((mu+h[1])/gamma), 0.38];
  m[1,] = [mu_ret, mu_hloffset + (mu+h[1])/2];
  
  for (t in 2:T) {
    // tau[t] = exp(alpha[1]*h[t-1] + alpha[2]*(h[t-1]^2-1));
    h[t] = phi * h[t-1] + sigma*v[t];
    
    s[t,] = [exp((mu+h[t])/gamma), 0.38];
    // m[t,] = [mu_ret + beta[1]*h[t-1] + beta[2]*(h[t-1]^2-1), mu_hloffset + (mu+h[t])/2] + R[1:2,3]'*v[t];
...
generated quantities {
  ...
  s_rep[1,] = [exp((mu+h_rep[1])/gamma), 0.38];
  m_rep[1,] = [mu_ret, mu_hloffset + (mu+h[1])/2];

  for(t in 2:T) {
    h_rep[t] = phi*h[t-1] + sigma*v_rep[t];

    s_rep[t,] = [exp((mu+h_rep[t])/gamma), 0.38];
