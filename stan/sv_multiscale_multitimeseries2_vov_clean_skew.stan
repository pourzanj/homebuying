...
transformed parameters {
  ...
  matrix[5,5] R = multiply_lower_tri_self_transpose(L);
  matrix[4,4] Rg = R[1:4,1:4] - quad_form_sym([[inv(R[5,5])]], [R[5,1:4]]);
  matrix[3,3] Rh = Rg[1:3,1:3] - quad_form_sym([[inv(Rg[4,4])]], [Rg[4,1:3]]);

  // Declare time variables
  vector[T] delta = (lambda * sqrt(U)) ./ sqrt(1 + lambda^2 * U);
  vector[T] u_raw = xi + (delta .* u_raw1 + sqrt(1-delta^2) .* u_raw2) .* sqrt(U);
  ...
  v[1] = sigma*exp(g[1]/2) * (R[4,5]*u_raw[1] + sqrt(Rg[4,4])*v_raw[1]);
  h[1] = v[1] / sqrt(1 - phi^2);
  
  S[1] = exp((mu+h[1])/2);
  s[1,] = [S[1], sigma_obs[1], sigma_obs[2]];
  m_corr[1,] = (R[1:3,5]*u_raw[1] + Rg[1:3,4]*inv_sqrt(Rg[4,4])*v_raw[1])';
  ...
  for (t in 2:T) {
    ...
    v[t] = sigma*exp(g[t]/2) * (R[4,5]*u_raw[t] + sqrt(Rg[4,4])*v_raw[t]);
    h[t] = phi*h[t-1] + v[t];
  
    S[t] = exp((mu+h[t])/2);
    s[t,] = [S[t], sigma_obs[1], sigma_obs[2]];
    m_corr[t,] = (R[1:3,5]*u_raw[t] + Rg[1:3,4]*inv_sqrt(Rg[4,4])*v_raw[t])';
...
generated quantities {
  vector[T] U_rep = to_vector(inv_gamma_rng(rep_vector(nu/2, T), nu/2));
  vector[T] delta_rep = (lambda * sqrt(U_rep)) ./ sqrt(1 + lambda^2 * U_rep);
  vector[T] u_raw1_rep = fabs(to_vector(normal_rng(rep_vector(0.0, T), 1.0)));
  vector[T] u_raw2_rep = to_vector(normal_rng(rep_vector(0.0, T), 1.0));
  vector[T] u_raw_rep = xi + (delta_rep .* u_raw1_rep + sqrt(1-delta_rep^2) .* u_raw2_rep) .* sqrt(U_rep);
  ...
  u_rep[1] = tau*u_raw_rep[1];
  g_rep[1] = u_rep[1] / sqrt(1-theta^2);
  
  v_rep[1] = sigma*exp(g_rep[1]/2) * (R[4,5]*u_raw_rep[1] + sqrt(Rg[4,4])*v_raw_rep[1]);
  h_rep[1] = v_rep[1] / sqrt(1 - phi^2);
  
  S_rep[1] = exp((mu+h_rep[1])/2);
  s_rep[1,] = [S_rep[1], sigma_obs[1], sigma_obs[2]];
  m_corr_rep[1,] = (R[1:3,5]*u_raw_rep[1] + Rg[1:3,4]*inv_sqrt(Rg[4,4])*v_raw_rep[1])';
  ...
  for (t in 2:T) {
    ...
    u_rep[t] = tau*u_raw_rep[t];
    g_rep[t] = theta*g[t-1] + u_rep[t];
  
    v_rep[t] = sigma*exp(g_rep[t]/2) * (R[4,5]*u_raw_rep[t] + sqrt(Rg[4,4])*v_raw_rep[t]);
    h_rep[t] = phi*h[t-1] + v_rep[t];
  
    S_rep[t] = exp((mu+h_rep[t])/2);
    s_rep[t,] = [S_rep[t], sigma_obs[1], sigma_obs[2]];
    m_corr_rep[t,] = (R[1:3,5]*u_raw_rep[t] + Rg[1:3,4]*inv_sqrt(Rg[4,4])*v_raw_rep[t])';
