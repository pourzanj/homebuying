...
```{r}
fit_egarch_sgt <-
  stan(file = "stan/egarch_sgt.stan",
...
print(fit_egarch_sgt, pars = c("omega", "phi", "k1", "k2", "lambda1",
                               "mu", "l", "p", "q"))
```

```{r}
s <- extract(fit_egarch_sgt)

link <- function(x) exp(x / 20)
...
```{r generate_process}
generate_egarch_sgt <- function(i, date) {
  ...
  tibble(sample_id = rep(i, N), date, h, lambda, y) %>%
...
sim_traj <-
  1:500 %>%
  sample(4) %>%
  map_dfr(generate_egarch_sgt, date = spx$date)
